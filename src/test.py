# test.py - 模型评估测试模块
import torch
from CHDataSet import CHDataSet
from torch.utils.data import DataLoader
from Model import Model
from transformers import BertTokenizer
# sklearn：Python 中最流行、最核心的传统机器学习库。模型评估行管哭
# precision_score：精确率 recall_score：召回率 accuracy_score：准确率 classification_report：多分类报告
...
# 加载字典和分词器
token = BertTokenizer.from_pretrained(r"/Users/chaors/Development/AIProjects/AIentimentAnalysis/model/bert-base-chinese/models--bert-base-chinese/snapshots/8f23c25b06e129b6c986331a13d8d025a92cf0ea")
...
def evaluate_model(model, test_loader, device):
    ...
    for i, (input_ids, attention_mask, token_type_ids, labels) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            # argmax：推理阶段针对CrossEntropyLoss模型输出的连续值转换为离散的类别决策，实现预测功能
            # argmax返回的是outputs中连续离散值的最大值下标。eg：
            # outputs = torch.tensor([[0.1, 0.6, 0.3], [0.5, 0.2, 0.3]])，则
            # preds = torch.argmax(outputs, dim=1)返回tensor([1, 0])。
            # 第一个样本：第1类概率最高（0.6）→ 预测为类别1(下标)；第二个样本：第0类概率最高（0.5）→ 预测为类别0(下标)
